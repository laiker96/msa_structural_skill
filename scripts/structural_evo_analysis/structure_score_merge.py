#!/usr/bin/env python3
"""Compute and merge solubility/Aggrescan3D structure scores."""
...
DEFAULT_OUT = cfg.OUTPUT_DIR / "structure_scores"
...
DEFAULT_CAMSOL_SCRIPT = ROOT / "scripts" / "structural_evo_analysis" / "structure_score_camsol.py"
DEFAULT_AGGRESCAN_SCRIPT = ROOT / "scripts" / "structural_evo_analysis" / "structure_score_aggrescan3d.py"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the local CamSol-style and Aggrescan3D scorers, then merge "
            "their per-residue/global outputs."
        )
    )
    ...
    parser.add_argument("--structure-validation", type=Path, default=None)
    parser.add_argument(
        "--validation-statuses",
...
def maybe_run_scorers(args: argparse.Namespace) -> None:
    if args.skip_compute:
        return
    validation_args = []
    if not args.skip_structure_validation_filter and args.structure_validation is not None:
        if not args.structure_validation.exists():
            raise SystemExit(
                f"missing structural validation table: {args.structure_validation}\n"
                "Provide a valid structural validation TSV, or use "
                "--skip-structure-validation-filter."
            )
        validation_args = [
