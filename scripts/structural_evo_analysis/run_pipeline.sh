...
if [[ $# -lt 1 ]]; then
  ...
  echo "Provide query PDB with SEA_QUERY_PDB or ${SEA_STRUCTURE_DIR:-${SEA_WORK_DIR:-${HOME}/structural_evo_analysis}/structures}/query.pdb." >&2
  exit 2
fi
...
if [[ ! -s "${QUERY_PDB}" ]]; then
  echo "ERROR: query PDB is required for vulnerability analysis: ${QUERY_PDB}" >&2
  exit 2
fi
