#!/usr/bin/env python3
"""MSA pipeline CamSol-style solubility scoring for ANKros structures.
...
This file is pipeline-owned so `15_compute_solubility_aggregability.py` can
recompute MSA HTML score inputs without depending on sibling experimental
script directories.
...
DEFAULT_OUT_DIR = ROOT / "results" / "solubility" / "camsol"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        help="Step 14 structural validation TSV; AFDB structures with excluded statuses are skipped.",
    )
    parser.add_argument(
        "--validation-statuses",
...
def input_structures(args: argparse.Namespace) -> list[Path]:
    structures = []
    if args.ankros_pdb.exists():
        structures.append(args.ankros_pdb)
...
Generated by `scripts/solubility/camsol.py`.
